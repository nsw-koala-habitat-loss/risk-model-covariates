...
# Woody_GDA94 <- project(Woody, crs(SA1_UCL), threads=TRUE, filename=file.path(INPUT_DIR, "Woody_GDA94.tif")
# 

SA1_UCL_rast <- rasterize(SA1_UCL, Woody, field = "SSR_CODE16")

DistCity <- terra::distance(SA1_UCL_rast, filename = file.path(OUTPUT_DIR, "DistCity.tif"), overwrite=TRUE)
