...
PlanZone <- vect("D:/Data/NSW_Deforestation/risk-model-covariates/Input/NSW_EPI_land_zones/All_EPI_Data_Shapefile_GDA94_22122022/EPI_Land_Zoning.shp")%>%
  project(crs(Woody)) %>% 
  ...
  tidyterra::mutate(Code = as.factor(Code)) %>% 
...
names(PlanZone)
ggplot()+geom_spatvector(data =PlanZone, aes(fill = Code))
